...
func TestHandler_processScrapeTask(t *testing.T) {
	type args struct {
		ctx  context.Context
		task *asynq.Task
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.h.processScrapeTask(tt.args.ctx, tt.args.task); (err != nil) != tt.wantErr {
				t.Errorf("Handler.processScrapeTask() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
